...
## Шаг 3. На jwt.io подделываем JWT под mgalankov@4274

Стандартно:
На jwt.io вставляем свой токен.
В VERIFY SIGNATURE → secret — wineyisthebest, видим зелёное «Signature Verified».
Меняем "sub" на "10", "username" — на "mgalankov@4274".
Копируем новый JWT.
...
Математически валидна (подпись для своего timestamp).
Лежит в нашей session['pending_signatures'].
...
JWT-секрет — словарный (wineyisthebest из rockyou). Подделываем JWT под mgalankov-а.
Flask SECRET_KEY — то же словарное слово. Подделываем Flask-session, кладём туда pending_signatures = { t: sig }.
Генератор подписей — самопальный truncated LCG mod 2^128 с публикацией только верхних 64 бит и фиксированными параметрами. По 20 подписям восстанавливается LLL-решёткой за секунды (а в этом CTF параметры ещё и доступны в исходниках).
После восстановления генератора предсказываем подпись для произвольного timestamp и сабмитим всё разом.
Mitigation

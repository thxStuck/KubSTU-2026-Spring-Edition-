...
Порт 9999 — передача пароля в открытом виде
Порт 31337 — передача зашифрованного ZIP-архива по собственному бинарному протоколу
...
В Wireshark: Follow TCP Stream (порт 31337), формат Raw, сохранить как файл raw_stream.bin
Отрезать первые 4 байта (магия "XFER")
Взять байты 4–19 — это XOR-ключ
Взять байты 20–23 — длина (big-endian)
Взять данные с байта 24 — зашифрованный архив
В CyberChef: XOR с ключом → скачать результат как .zip
Распаковать с паролем IcyFl1pp3r$2026

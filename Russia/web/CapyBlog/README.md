...
обнаруживаем бекап веб приложения по пути `/backup/www.zip`
...
__wakeup() - вызывается СРАЗУ при десериализации.
__destruct() - вызывается, когда объект удаляется из памяти (конец выполнения скрипта).
__toString() - вызывается, если объект пытаются использовать как строку.
...
Выбрать имя файла (например, в корне веб приложения /var/www/html/
Сформировать PHP-код (шелл).
Запаковать это в формат, понятный unserialize().
Логика эксплоита:
